...
namespace Azure.DependencyInjection.Services
{
    internal class ServiceProviderHolder
    {
        ...
        {
            if (_scopes.TryRemove(functionInstanceId, out var scope)) scope.Dispose();
        }

        public object GetRequiredService(Guid functionInstanceId, Type serviceType)
        {
            var scopeFactory = _serviceProvider.GetService<IServiceScopeFactory>();
            if (scopeFactory == null) throw new InvalidOperationException("The current service provider does not support scoping!");
